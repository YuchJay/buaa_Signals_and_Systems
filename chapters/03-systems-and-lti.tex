...
\end{equation}
研究系统性质时，应检查\red{框中固定的输入输出规则}，不能只看一组特定的输入、输出波形。下文若写“$x$ 产生 $y$”，均指 $x$ 经过同一个待研究系统后得到 $y$。
...
\begin{intuition}
  把输入拆成若干简单分量，分别求响应后相加，结果必须与完整输入的响应相同。后面用冲激分解研究 LTI 系统，靠的就是这一点。
\end{intuition}
...
\begin{example}[增益、微分与累加]
  ...
  \]
  同样满足\cref{eq:linearity-test}。判断线性应看\red{运算对线性组合的作用}；公式的外观是否复杂没有影响。
\end{example}
...
这里的“记忆”专指计算当前输出时\red{是否需要当前时刻以外的输入}，与系统内部有没有存储器无关。$y(t)=t x(t)$ 虽然显含时间，仍然无记忆。
...
\begin{example}[逐点非线性与延迟]
  ...
  \[
    y(t)=x^2(t)+\e^{x(t)}
  \]
  只使用当前值 $x(t)$，所以无记忆。$y(t)=x(t-1)$ 读取过去值，故有记忆。离散时间中，$y[n]=x^2[n]$ 无记忆；$y[n]=x[2n]$ 在多数下标处读取 $x[n]$ 以外的样值，所以有记忆。
\end{example}
...
\begin{pitfall}[只检查原点或个别下标]
  对 $y[n]=x[2n]$，在 $n=0$ 时确有 $y[0]=x[0]$，但系统性质要求对任意时刻成立。取 $n=1$，输出需要 $x[2]$，已经足以判定系统有记忆。检查内部输入下标与当前输出下标是否对\red{所有时刻恒相等}即可。
\end{pitfall}
...
可逆系统必须\red{保留区分输入所需的信息}。若能从一般的 $y$ 唯一解出 $x$，恢复公式就是逆系统；若两个不同输入产生同一输出，系统不可逆。
...
\begin{example}[累加与差分]
  ...
  \[
    x[n]=y[n]-y[n-1].
  \]
  因而在累加和存在的信号类上，它的逆系统是后向差分。验证逆系统时，应\red{把恢复公式代回原关系}。
\end{example}
...
证明稳定性，需要从任意输入界 $M_x$ 推出一个与时间无关的有限输出界；否定稳定性，只需找到一个使输出无界的有界输入。可概括为\red{证明要一般、否定用反例}。
...
\begin{example}[微分、积分与差分]
  理想微分系统不满足 BIBO 稳定性：有界阶跃输入的导数包含单位冲激，输出不再是有界普通函数。积分器也不稳定；取有界输入 $x(t)=\stepfun(t)$，则
  \[
  ...
  \]
  所以稳定。系统名称不能决定稳定性；应检查它会不会把有界变化\red{累积或放大为无界输出}。
\end{example}
