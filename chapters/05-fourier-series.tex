...
傅里叶级数把周期信号展开成一组彼此正交的正弦或复指数。展开系数给出离散谐波线谱，后面的傅里叶变换、LTI 系统频率响应和滤波分析都从这里出发。

\section{数学准备}
\label{sec:fourier-inner-product-foundations}

内积、正交基、投影系数和 Gram--Schmidt 正交化见第一章\cref{sec:inner-product-foundations}。这里直接把这些结果用于周期区间上的傅里叶基。
...
\end{align}
其中 $B_0$ 是一个周期内的平均值，$B_n,C_n$ 分别记录余弦和正弦谐波的权重。取积分区间时最重要的是\red{完整覆盖一个周期}，不一定要从 $0$ 开始。

\subsection{收敛条件与跳变点}

常用的 Dirichlet 条件可概括为：
\begin{itemize}
  \item 在一个周期内，信号应绝对可积；
  \item 只有有限个极值；
  \item 只有有限个有限幅度的间断点。
...
傅里叶基适合周期且具有全局振荡结构的信号。在此补充其他几类的正交展开：
...
这些方法都先选择正交方向，再计算投影系数并重构信号。下一章把离散的谐波索引推广为连续频率变量。
